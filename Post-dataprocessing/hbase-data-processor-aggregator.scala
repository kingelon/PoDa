...
object HBaseDataProcessor {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (args.length != 1) {
      println("Usage: HBase Data Processor <config-file-path>")
      System.exit(1)
    }
    ...
    // Step 4: Aggregation - Compute Metrics on Specific Fields
    val aggregatedDF = sourceDF.groupBy("deviceClass")
      .agg(
        count("rowkey").alias("deviceCount"),
        avg(length(col("userAgent"))).alias("avgUserAgentLength")
      )
    println("Aggregated Data Preview:")
    aggregatedDF.show(5)

    // Step 5: Data Transformation - Generate Hashes for Fingerprinting
    val processedDF = aggregatedDF.withColumn(
      "deviceFingerprint",
      hashUDF(
        concat_ws(",",
          col("deviceClass"), col("avgUserAgentLength")
        )
      )
    )
    ...
    // Step 6: Write Processed Data Back to HBase
  ...
  def writeHBaseTable(df: DataFrame, namespace: String, tableName: String): Unit = {
    df.write.format("org.apache.hadoop.hbase.spark")
      .option("hbase.table", s"$namespace:$tableName")
      .option("hbase.columns.mapping", "rowkey STRING :key, cf:deviceFingerprint STRING, cf:deviceCount STRING, cf:avgUserAgentLength STRING")
      .save()
  }
